...
export function ShoppingCartDrawer({ isVisible, setIsVisible }: ShoppingCartDrawerProps) {
	return (
...
export function ShoppingCartDrawerContent() {
	const { products, changeProductAmount, handleRemoveFromShoppingCart, handleCloseDrawer } = useShoppingCart();
	const totalProducts = useMemo(() => formatPrice(products.map((x) => x.amount * x.price).reduce((pv, cv) => pv + cv, 0)), [products]);
	return (
					...
					<div
						...
						<div className="flex-1 p-4 flex flex-col justify-between gap-4">
							<div className="flex justify-between items-center">
								...
								<button type="button" onClick={() => handleRemoveFromShoppingCart(product.id)}>
									<Image width={20} height={20} src={XCircleIcon} alt="X" />
								</button>
							</div>
							<div className="flex justify-between items-center">
								<Counter
									handleAdd={() => changeProductAmount(product.id, product.amount + 1)}
									handleRemove={() => changeProductAmount(product.id, product.amount - 1)}
									total={product.amount}
								/>
								<span className="text-xl text-custom-violet">{formatPrice(product.price)}</span>
							</div>
						</div>
					</div>
				))}
