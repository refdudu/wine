...
export class ProductRepositoryJson implements ProductRepositoryI {
	...
	public getTotal({ betweenPrices = "" }: GetProductsFilter): number {
		if (betweenPrices) {
			...
			return this.products.filter((product) => this.filterProduct(product, min, max)).length;
		}
		return this.products.length;
	}
	private getFilteredProducts({ betweenPrices = "", searchText = "" }: GetProductsFilter) {
		if (betweenPrices) {
			const [min, max] = betweenPrices.split("-");
			this.products = this.products.filter((product) => this.filterProduct(product, min, max));
		}
		if (searchText) {
			const normalize = (text: string) => text.normalize("NFD").toLocaleLowerCase();
			searchText = normalize(searchText);
			this.products = this.products.filter((x) => normalize(x.name).includes(searchText));
		}
